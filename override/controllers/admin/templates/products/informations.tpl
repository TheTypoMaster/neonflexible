	...
	<div class="col-lg-9">
		...
		<div class="radio">
			<label for="pack_product">
				<input type="radio" name="type_product" {if $is_in_pack}disabled="disabled"{/if} id="pack_product" value="{Product::PTYPE_PACK}" {if is_null($oNowProductTypeProduct->id_now_product_type) && $product_type == Product::PTYPE_PACK}checked="checked"{/if} />
				{l s='Pack of existing products'}
			</label>
		</div>
		<div class="radio">
			<label for="virtual_product">
				<input type="radio" name="type_product" id="virtual_product" {if $is_in_pack}disabled="disabled"{/if} value="{Product::PTYPE_VIRTUAL}" {if is_null($oNowProductTypeProduct->id_now_product_type) && $product_type == Product::PTYPE_VIRTUAL}checked="checked"{/if} />
				{l s='Virtual product (services, booking, downloadable products, etc.)'}
			</label>
		</div>

		{foreach $aNowProductTypes as $nowProductType}

			<div class="radio">
				<label for="now_product_type_{$nowProductType['id_now_product_type']}">
					<input type="radio" name="type_product" id="now_product_type_{$nowProductType['id_now_product_type']}" value="id_now_product_type_{$nowProductType['id_now_product_type']}" {if $oNowProductTypeProduct->id_now_product_type == $nowProductType['id_now_product_type']}checked="checked"{/if} />
					{$nowProductType['name']}
				</label>
			</div>
