					...
					<li class="ajax_block_product{if $smarty.foreach.accessories_list.first} first_item{elseif $smarty.foreach.accessories_list.last} last_item{else} item{/if} product_accessories_description">
						...
						<div class="content-right">
							...
							{if !$PS_CATALOG_MODE && ($accessory.allow_oosp || $accessory.quantity > 0)}
								<a class="button-add-to-cart gradient ajax_add_to_cart_button" href="{$link->getPageLink('cart', true, NULL, "qty=1&amp;id_product={$accessory.id_product|intval}&amp;token={$static_token}&amp;add")|escape:'html'}" data-id-product="{$accessory.id_product|intval}" title="{l s='Add to cart'}">
									<span></span>
									{l s='Add to cart'}
								</a>
							{/if}
