	...
	<form action="{$link->getPageLink($back_order_page, true)|escape:'html'}" method="post">
		...
		<div id="opc_account" class="opc-main-block">
			...
			<div class="addresses clearfix">
				...
				<div class="clearfix">
					<ul class="block" id="address_delivery" {if $cart->isVirtualCart()}style="display:none;"{/if}>
					</ul>
					<ul class="block {if $cart->isVirtualCart()}full_width{/if}" id="address_invoice">
					</ul>
				</div>
			...
			<p class="cart_navigation submit">
				...
				<a href="{$link->getPageLink($back_order_page, true, NULL, "step=0{if $back}&back={$back}{/if}")|escape:'html'}" title="{l s='Previous'}" class="button-rose left">&laquo; {l s='Previous'}</a>
				<input type="submit" name="processAddress" value="{l s='Next'} &raquo;" class="button-rose right" />
			</p>
	</form>
	{else}
</div>
