		...
		public function uninstallSqlFiles() {
			...
			if (!empty($this->aSqlFileToUninstall)) {
				foreach ($this->aSqlFileToUninstall as $sVersion => $sFileName) {
					$bReturn &= $this->executeSqlFile($sVersion, $sFileName, true);
				}
			}
			return $bReturn;
		}

		public function executeSqlFile($sVersion, $sFileName, $uninstall = false) {

			if ($uninstall || Tools::version_compare($sVersion, $this->version, '=')) {
