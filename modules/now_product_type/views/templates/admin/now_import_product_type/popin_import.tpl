		...
		<form action="{$current}&token={$token}" method="post" enctype="multipart/form-data">
			<div class="form-group">
				<label for="file" class="control-label col-lg-3">
					<span class="label-tooltip" data-toggle="tooltip" title="{l s='Select your CSV file' mod='now_product_type'}">
						{l s='Select your CSV file' mod='now_product_type'}
					</span>
				</label>
				<div class="col-sm-9">
					<div class="row">
							...
							<div class="dummyfile input-group">
								...
								<span class="input-group-btn">
									<button id="file-selectbutton" type="button" name="submitAddAttachments" class="btn btn-default">
										<i class="icon-folder-open"></i> {l s='Choose a file' mod='now_product_type'}
									</button>
								</span>
							</div>
						</div>
			...
			<div class="form-group">
				...
				<input type="submit" name="submitFileUpload" value="{l s='Upload' mod='now_product_type'}" class="button" />
				<p class="preference_description">{l s='Only UTF-8 and ISO-8859-1 encoding are allowed' mod='now_product_type'}</p>
			</div>
		</form>
	</div>
</div>